"""Compose every Blush Matcha store asset from one HTML/CSS source.

Layering and geometry mirror a real installed-Chrome screenshot (1080x675
capture space). The window is authored in that pixel space and rasterised at a
matching device scale factor so the store shot stays crisp:
...
Chrome-painted UI colors are hardcoded and marked with their source.
"""
...
C['deep'] = [58, 50, 42]        # warm ink for AA text on the blush pink areas
...
# ---------------------------------------------------------------------------
# UI colors NOT controlled by the theme: painted by Chrome itself, so they are
# hardcoded from a real install whenever one is available.
# ---------------------------------------------------------------------------
# All values below are sampled from the user's real Chrome window with this
# theme installed (1080x650 capture, 2026-09-20). Chrome derives both the new
# tab mark and the shortcut tiles from ntp_background: hue and saturation are
# kept, lightness is clamped for contrast (very light tinted bg -> L~0.69,
# near-neutral bg -> L~0.66).
LOGO = '#EF6F9F'          # real: Google mark on the new tab page
TILE = '#F49CBD'          # real: shortcut circle fill
OMNI_BG = '#E0CAD5'       # real: omnibox fill (Chrome blends it against the toolbar)
UI_TEXT = '#5F6368'       # placeholder / shortcut label grey
UI_ICON = '#444746'       # mic, bookmark, apps glyphs (sampled)
PILL_BG = '#202124'       # "Customize Chrome" pill fill (sampled)
PILL_FG = '#8AB4F8'       # its label (sampled)
WIN_BTN = '#3A322A'       # minimize / maximize / close glyphs on the blush frame
G_RED, G_BLUE, G_YELLOW, G_GREEN = '#EA4335', '#4285F4', '#FBBC05', '#34A853'

VARS = f""":root{{
  --blush:{color('frame')};
  --matcha:{color('toolbar')};
  --milk:{color('ntp_background')};
  --ink:{color('ntp_text')};
  --green:{color('tab_background_text')};
  --deep:{color('deep')};
...
.tabstrip{height:32px;background:var(--blush);display:flex;align-items:flex-end;padding-left:33px;position:relative}
...
.tab{width:168px;height:27px;border-radius:9px 9px 0 0;margin-right:7px;padding:0 10px 0 30px;display:flex;align-items:center;
     font-size:11.5px;color:var(--green);position:relative;outline:1px solid rgba(255,255,255,.38);outline-offset:-1px}
.tab.on{background:var(--matcha);outline:0;color:var(--ink)}
.tab i{position:absolute;left:11px;top:7px;width:12px;height:12px;border-radius:3px;background:rgba(58,50,42,.18)}
...
/* ---- toolbar ---- */
.toolbar{height:32px;background:var(--matcha);display:flex;align-items:center;gap:16px;padding:0 14px}
.nav{display:flex;gap:15px;align-items:center}
.omni{width:660px;height:28px;border-radius:14px;background:var(--omni-bg);display:flex;align-items:center;
      padding:0 11px;gap:9px;font-size:12.5px;color:var(--ui-text)}
.omni .ph{flex:1;white-space:nowrap;overflow:hidden}
.tools{display:flex;gap:17px;align-items:center;margin-left:auto}

/* ---- bookmark bar ---- */
.bookmarks{height:32px;background:var(--matcha);display:flex;align-items:center;gap:19px;padding:0 14px;
...
.tile{width:440px;height:280px;background:var(--blush);position:relative;overflow:hidden;text-align:center;color:var(--deep)}
.tile img{width:88px;height:88px;display:block;margin:26px auto 0;
          filter:drop-shadow(0 5px 14px rgba(58,50,42,.16))}
.tile h1{font-family:Georgia,serif;font-weight:normal;font-size:37px;margin:12px 0 0}
.tile .kicker{font-size:12px;letter-spacing:5px;margin-top:8px;color:var(--green)}
.tile p{font-size:14px;margin:16px 0 0;color:var(--deep)}
.tile:after{content:'';position:absolute;left:0;right:0;bottom:0;height:14px;background:var(--matcha)}
...
.marquee .frame{position:absolute;top:139px;left:300px;width:800px;height:370px;overflow:hidden;border:2px solid var(--blush);
                border-radius:16px;box-shadow:0 10px 30px rgba(58,50,42,.13)}
.marquee .frame .window{transform:scale(.740741);transform-origin:top left}

/* ---- screenshot 2: palette card ---- */
/* neutral white page so the petal-white card (#FEF6F9) reads as a card */
.intro{width:1280px;height:800px;background:#FFFFFF;padding:66px 74px}
.intro .kicker{font-size:12px;letter-spacing:4px;color:var(--green)}
...
def window(height=675, tabs=('Reading list', 'Design inspiration', 'Blush Matcha', 'New Tab')):
    ...
    # real capture shows "Images" + the apps grid only (no Gmail link, no avatar)
...
tile = ('<div class="tile">'
        + f'<img src="{LOGO_URI}" alt="Blush Matcha logo">'
        + '<h1>Blush Matcha</h1><div class="kicker">CHROME THEME</div>'
        + '<p>Blush pink and pale matcha, side by side.</p></div>')
marquee = ('<div class="marquee"><h1>Blush Matcha Theme</h1>'
           '<p>Blush pink, pale matcha, and a calm new tab.</p>'
           + '<div class="frame">' + window(height=500) + '</div></div>')

PALETTE = [
    ('Matcha Frame', 'frame', 'Window frame', 'deep'),
    ('Blush Toolbar', 'toolbar', 'Toolbar & active tab', 'deep'),
    ('Petal White', 'ntp_background', 'New tab page', 'deep'),
    ('Deep Matcha', 'tab_background_text', 'Tab labels & body text', '#FFFFFF'),
]
cards = ''.join(
    f'<div class="card" style="background:{color(k)};color:{fg if fg.startswith("#") else color(fg)};'
    f'border:1px solid rgba(58,50,42,.16)">'
    f'<strong>{name}</strong><span>{color(k)} · {role}</span></div>' for name, k, role, fg in PALETTE)
intro = ('<div class="intro"><div class="kicker">SOFT BLUSH, QUIET MATCHA</div><h1>Blush Matcha Theme</h1>'
         '<p class="lead">Four colors. One calm, fresh space.</p><div class="cards">' + cards + '</div>'
         '<p class="chips">Solid colors · Soft contrast · Calm new tab</p></div>')
